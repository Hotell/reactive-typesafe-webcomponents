...
const template = ({ name, age, tricks }: Props) => html`
  ${css}
  <header>
  ...
  ${html`<form autocomplete="off" on-submit=${(ev: Event) => {
    ev.preventDefault
    console.log(ev)
  }}>`}
    ...
    <button type="submit" on-click=${function(e: Event) {
      console.log(e)
    }}>Learn Trick</button>
  </form>
  <div>
    <h4>User knows following tricks:</h4>
    <ul id="trick-list">${tricks.map(trick => html`${trick.name} | ${trick.difficulty}`)}</ul>
  </div>
`
...
export class User extends HTMLElement implements Props {
  ...
  render() {
    ...
    render(template(props), this.shadowRoot!)
  }
